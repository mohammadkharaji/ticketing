...
export default function ExpertDashboard() {
  ...
  useEffect(() => {
    async function fetchTickets() {
      setLoading(true);
      try {
        const res = await ticketService.getTickets({ assigneeId: userProfile?.id, searchTerm: search });
  ...
  // محاسبه عملکرد شخصی
  const resolvedCount = stats.closed;
    ...
    <MainLayout>
      <div className="space-y-6">
        ...
        <Card>
          ...
          <CardContent>
            <div className="flex items-center gap-4">
              <span className="text-sm text-muted-foreground">شما به عنوان کارشناس وارد شده‌اید.</span>
        ...
        <div className="flex items-center gap-4 flex-wrap">
          <Input
            placeholder="جستجو در تیکت‌های ارجاع‌شده..."
            value={search}
        ...
        {/* لیست آخرین تیکت‌های ارجاع‌شده */}
        <Card>
          <CardHeader>
            <CardTitle>آخرین تیکت‌های ارجاع‌شده به من</CardTitle>
          </CardHeader>
          <CardContent>
            {loading ? (
              <div>در حال بارگذاری...</div>
            ) : filteredTickets.length === 0 ? (
              <div className="text-sm text-muted-foreground">تیکتی ارجاع نشده است.</div>
            ) : (
        ...
        <Card>
          <CardHeader>
            <CardTitle>عملکرد من</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="flex gap-4">
              <div>
                <span className="font-bold">{resolvedCount}</span> تیکت حل‌شده
              </div>
              <div>
                <span className="font-bold">{totalCount}</span> تیکت ارجاع‌شده
              </div>
            </div>
          </CardContent>
        </Card>
