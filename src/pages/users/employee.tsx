...
export default function EmployeeDashboard() {
  ...
  useEffect(() => {
    async function fetchTickets() {
      setLoading(true);
      try {
        const res = await ticketService.getTickets({ creatorId: userProfile?.id, searchTerm: search });
  ...
  return (
    <MainLayout>
      <div className="space-y-6">
        ...
        <Card>
          ...
          <CardContent>
            <div className="flex items-center gap-4">
              <span className="text-sm text-muted-foreground">شما به عنوان کارمند وارد شده‌اید.</span>
        ...
        {/* لیست آخرین تیکت‌های ثبت‌شده */}
        <Card>
          <CardHeader>
            <CardTitle>آخرین تیکت‌های من</CardTitle>
          </CardHeader>
          <CardContent>
            {loading ? (
